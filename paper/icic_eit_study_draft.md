...
We also evaluated the partially trained pulmonary FCUNet control run on the same test split. Even before full completion, the best currently saved checkpoint reached **0.7815** mean score and **160.99** total score, clearly surpassing the pulmonary DCT variants. This finding is important because it shows that the representation ranking is task-dependent: the fixed-basis DCT approach is strongest on the benchmark phantom task, but the more flexible FCUNet currently performs better on the lung-style synthetic dataset. In other words, the pulmonary regime appears to reward a richer spatial decoder more than the benchmark phantom regime does, even though FCUNet is more expensive to train.
