...
import matplotlib
# use non-interactive backend for saving files in scripts
matplotlib.use('Agg')
...
def main(inputFolder, outputFolder, categoryNbr):
    ...
    for objectno in range(0, len(mat_files)):  # 对每个输入文件计算重建
        ...
        reco_png = os.path.join(outputFolder, f"{objectno + 1}_reco.png")
        ...
        seg_png = os.path.join(outputFolder, f"{objectno + 1}_seg.png")
        fig2.savefig(seg_png, bbox_inches='tight')
        plt.close(fig2)


if __name__ == '__main__':
    # Compute an absolute path to the example evaluation dataset relative to this script.
    # main.py is located at: data/Codes_Python/Codes_Python/main.py
    # We go up two levels to reach the data/ folder and then into EvaluationData/.../level1
    repo_data = Path(__file__).resolve().parents[2] / 'EvaluationData' / 'EvaluationData' / 'evaluation_datasets' / 'level1'
    main(str(repo_data), 'results', 1)