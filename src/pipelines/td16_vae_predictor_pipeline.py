"""Inference pipeline for TD16 VAE-latent pulmonary delta-sigma prediction."""
...
class TD16VAEPredictorPipeline(BasePipeline):
    ...
    def load_model(self, level: int = 1) -> None:
        ...
        if not self.config.vae.checkpoint:
            raise ValueError('TD16 VAE predictor config missing vae.checkpoint')
        vae = ConvVAE(
    ...
    def reconstruct_batch(self, Uels, ref_data=None, level=1):
        ...
        with torch.no_grad():
            with self._autocast_context():
                latent = self.model(y_tensor)
                sigma = self.vae.decode(latent)
