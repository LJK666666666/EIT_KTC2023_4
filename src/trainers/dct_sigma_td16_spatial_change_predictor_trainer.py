"""Trainer for spatial-change-aware 16-electrode pulmonary TD16 regression."""
...
from ..configs.dct_sigma_td16_spatial_change_predictor_config import (
...
from ..models.dct_predictor import SpatialChangeGatedDCTPredictor


class DCTSigmaTD16SpatialChangePredictorTrainer(BaseTrainer):
    def __init__(self, config=None,
                 experiment_name='dct_sigma_td16_spatial_change_predictor_baseline'):
        if config is None:
    ...
    def build_model(self):
        ...
        print(
            f'DCTSigmaTD16SpatialChangePredictor: {total_params / 1e6:.2f}M parameters'
        )

    def build_datasets(self):
        if not self.config.data.get('use_hdf5', False):
            raise ValueError(
                'DCT sigma TD16 spatial change predictor requires HDF5 dataset.')
    ...
    def train_step(self, batch):
        ...
        with self._autocast_context():
            pred_sigma, coeffs, mask_logits = self.model(measurements, levels_tensor)
    ...
    def _collect_metrics(self, loader):
        ...
        for measurements, sigma_delta, domain_mask, *rest in loader:
            ...
            with torch.no_grad():
                with self._autocast_context():
                    pred_sigma, coeffs, mask_logits = self.model(
    ...
    def validate(self, epoch):
        ...
        print(
            ...
            + f' MaskAcc: {metrics["val_mask_acc"]:.6f}'
        )
        return metrics
