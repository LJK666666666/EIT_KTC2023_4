"""Trainer for 16-electrode pulmonary time-difference conductivity regression."""
...
from ..configs.dct_sigma_td16_predictor_config import (
...
class DCTSigmaTD16PredictorTrainer(BaseTrainer):
    def __init__(self, config=None, experiment_name='dct_sigma_td16_predictor_baseline'):
        if config is None:
    ...
    def build_model(self):
        ...
        print(f'DCTSigmaTD16Predictor: {total_params / 1e6:.2f}M parameters')

    def build_datasets(self):
        if not self.config.data.get('use_hdf5', False):
            raise ValueError('DCT sigma TD16 predictor requires HDF5 dataset.')
