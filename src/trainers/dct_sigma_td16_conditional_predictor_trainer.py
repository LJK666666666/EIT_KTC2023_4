...
from ..configs.dct_sigma_td16_conditional_predictor_config import (
...
from ..models.dct_predictor import ConditionalSpatialChangeDCTPredictor


class DCTSigmaTD16ConditionalPredictorTrainer(BaseTrainer):
    def __init__(self, config=None,
                 experiment_name='dct_sigma_td16_conditional_predictor_baseline'):
        if config is None:
    ...
    def build_model(self):
        ...
        print(
            f'DCTSigmaTD16ConditionalPredictor: {total_params / 1e6:.2f}M parameters'
        )

    def build_datasets(self):
        if not self.config.data.get('use_hdf5', False):
            raise ValueError(
                'DCT sigma TD16 conditional predictor requires HDF5 dataset.')
        ...
        if oversample_factor > 1.0 and train_indices is not None:
            ...
            active = np.any(np.abs(sigma_delta) > threshold, axis=(1, 2))
    ...
    def _mask_target(self, target_sigma, domain_mask):
        ...
        threshold = float(self.config.training.get('mask_threshold', 0.02))
        return ((target_sigma.abs() > threshold).float() * mask)
    ...
    def _maybe_teacher_force_mask(self, target_mask_coeffs):
        ratio = float(self.config.training.get('mask_teacher_forcing_ratio', 0.0))
        if ratio <= 0.0:
            return None
        if ratio >= 1.0:
            return target_mask_coeffs
        use_teacher = torch.rand(1, device=target_mask_coeffs.device).item() < ratio
        return target_mask_coeffs if use_teacher else None
    ...
    def train_step(self, batch):
        ...
        with self._autocast_context():
            target_mask_coeffs = self.model.target_mask_coeffs(mask_target)
            mask_override = self._maybe_teacher_force_mask(target_mask_coeffs)
            pred_sigma, coeffs, mask_logits, mask_coeffs_pred = self.model(
                measurements, levels_tensor, mask_coeffs_override=mask_override
            )
            ...
            mask_loss = F.binary_cross_entropy_with_logits(
                mask_logits.float(), mask_target.float())
            mask_coeff_loss = F.mse_loss(
                mask_coeffs_pred.float(), target_mask_coeffs.float())
            total_loss = (
                image_loss +
                float(self.config.training.get('coeff_loss_weight', 0.25)) * coeff_loss +
                float(self.config.training.get('lambda_mask', 0.5)) * mask_loss +
                float(self.config.training.get('lambda_mask_coeff', 0.1)) * mask_coeff_loss
            )
        total_loss.backward()
        ...
        mask_acc = (mask_pred == mask_target).float().mean()
        return {
            ...
            'mask_coeff_loss': mask_coeff_loss.item(),
    ...
    def _collect_metrics(self, loader):
        ...
        for measurements, sigma_delta, domain_mask, *rest in loader:
            ...
            totals['mask_loss'] += mask_loss.item() * batch_size
            totals['mask_coeff_loss'] += mask_coeff_loss.item() * batch_size
            totals['mask_acc'] += mask_acc.item() * batch_size
        ...
        return {
            ...
            'mask_loss': totals['mask_loss'] / max(num_samples, 1),
            'mask_coeff_loss': totals['mask_coeff_loss'] / max(num_samples, 1),
            'mask_acc': totals['mask_acc'] / max(num_samples, 1),
            'preds': pred_np,
            'targets': target_np,
            'masks': mask_np,
            'reg': reg,
    ...
    def validate(self, epoch):
        ...
        metrics = {
            ...
            'val_mask_loss': metrics_raw['mask_loss'],
