...
将每个电极的76个测量值和角度余弦、正弦合起来作为78个输入值，乘上一个78*d的K/V得到d维向量，然后将每个位置的归一化横纵坐标、与中心点距离、角度余弦正弦共5个值输入一个MLP中得到d维向量后得到q。(拆分成多头)将q点乘k除以根号d再softmax后乘上V得到最终d张N*N特征图。然后卷积、最大最小池化、卷积、最大最小池化等等，然后反卷积等等，并且上下采样中同尺寸特征图进行连接，即U-Net，最后得到一张预测的图像。
